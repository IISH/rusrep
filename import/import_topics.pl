...
while (<>)
{
   ...
   if ($str=~/^(.+?)\s+(\d+\.\d+)\:\s*(.+)$/)
   {
	my ($topic, $datatype, $subtopic) = ($1, $2, $3);
	$topics{$topic}{$datatype} = $subtopic;
...
foreach $topic (sort keys %topics)
{
   if ($topic)
   {
	$dbh->do("insert into datasets.topics (topic_name, datatype, topic_root, description) values ('$topic', '$types{$topic}', '0', ' ')");
	$known{$topic}++;
   }
   my %datatypes = %{$topics{$topic}}; 
   foreach $datatype (sort keys %datatypes)
   {
	$dbh->do("insert into datasets.topics (topic_name, datatype, topic_root, description) values ('$datatypes{$datatype}', '$datatype', $types{$topic}, ' ')");
   }
